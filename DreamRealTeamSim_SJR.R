#This script is for computing the similarity of Dream Teams to Real Teams while SJR is considered as the success measure
...
Retrieve_Teams_from_Database<-function(folder_path, current_year, future_year){
  ...
  query<-"select researchareafor.resource_ID as team_concept,information_resource.venue,group_concat(DISTINCT researchareafor.concept_id) as concepts,("
  ...
  journal_information<-read.csv(paste(folder_path,'/Input Information/Journal_Information.csv',sep=""), header=TRUE, stringsAsFactors = FALSE)  
  title<-journal_information[,2]
  SJR<-journal_information[,ncol(journal_information)]
  min_value<-min(SJR, na.rm=TRUE)
  SJR[is.na(SJR)]<-min_value
  SJR1<-rep(0,0)
  
  for(kk in 1:length(venue)){
    
    index<-which(gsub("[[:punct:]]",'',toupper(title))==gsub("[[:punct:]]",'',toupper(venue[kk])))[1]
    
    if(!is.na(index))
      SJR1<-c(SJR1,SJR[index])  
    else
      SJR1<-c(SJR1,min_value)
    
  }
  
  
  return (list(SJR=SJR1,citedBy=citedBy,teams=team_concept,concepts_ID=concepts_ID,author=author,team_author=team_author))
}
